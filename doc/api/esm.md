...
Prior to the introduction of Node.js’ support for ES modules, it was a common
...
1. Document a new ES module entry point that’s not the package `"main"`, e.g.
   `import pkg from 'pkg/module.mjs'` (or `import 'pkg/module'`, if using
   [package exports][]). The package `"main"` would still point to a CommonJS
   file, and thus the package would remain compatible with older versions of
   Node.js that lack support for ES modules. For compatibility with other tools,
   it would be a good idea to additionally define the ES module entry point in
   the `package.json` `"module"` field (even though Node.js doesn’t acknowledge
   that field).
