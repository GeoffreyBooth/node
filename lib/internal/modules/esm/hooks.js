...
class HooksProxy {
  ...
  constructor() {
    ...
    const { InternalWorker, isMainThread, hooksPort } = require('internal/worker');
    ...
    if (isMainThread) {
      ...
      const { port1: forMainThread, port2: forHooksThread } = commsChannel;
      const worker = new InternalWorker(loaderWorkerId, {
        stderr: false,
        stdin: false,
        stdout: false,
        trackUnmanagedFds: false,
        transferList: [forHooksThread],
        workerData: {
          listenTo: forHooksThread,
          lock,
        },
      });
      worker.unref(); // ! Allows the process to eventually exit.
      worker.on('exit', process.exit);
      this.#worker = forMainThread;
    } else {
      this.#worker = hooksPort;
      // This prevents some erroneous behavior around using internals from worker_threads
      // on a normal message port waiting for a worker to finish initializing.
      this.waitForWorker = () => {};
    }
  }

  /**
   * Synchronously waits for the worker to fully initialize and have its entry point started up.
   */
  waitForWorker() {
    if (!this.#isReady) {
  ...
  async makeAsyncRequest(method, transferList, ...args) {
    process._rawDebug('getting worker');
    this.waitForWorker();
    process._rawDebug('got worker');
    ...
    process._rawDebug('sent message to worker');
    ...
    process._rawDebug('got message from worker');
