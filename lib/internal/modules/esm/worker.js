...
const allUserThreadPorts = []
/**
 * @param {MessagePort} toUserThreadPort
 */
function doListenTo(toUserThreadPort) {
  allUserThreadPorts.push(toUserThreadPort);
...
async function customizedModuleWorker(lock, syncCommPort, errorHandler) {
...

  {
...

    {
      ...
      process.exit = function(code) {
        for (const toUserThreadPort of allUserThreadPorts) {
          toUserThreadPort.postMessage(wrapMessage('exit', code ?? process.exitCode));
        }
   ...
   * TODO: make this use allUserThreadPorts instead of syncCommPort
   */
  ...
  process.on('beforeExit', () => {
    ...
    // TODO: Unclear the exact semantics but likely this should be to allUserThreadPorts?
    // TODO: Might be easier to just ref MessagePorts instead of this complexity?
    syncCommPort.on('message', handleMessage);
   ...
   * TODO: ports are being used oddly here, moved from closure to this value to
   *   better handle doListenTo & allUserThreadPorts integration but likely isn't 100% right
   *   things like the if (port == null ...) don't match logic beneath it doing syncCommPort
   *   (now `this`) work
   * @this {MessagePort}
  ...
  async function handleMessage({ method, args, port }) {
    ...
    // TODO: Refactor to not be a special edge case; Hooks class is too decoupled to do this easily right now
    if (method === '#listenTo') {
      /**
       * @type {MessagePort} A message port to listen to requests from the user thread
       */
      const toUserThreadPort = args[0]
      doListenTo(toUserThreadPort);
      return;
    }
    ...
    unsettledResponsePorts.add(port ?? this);
    ...
    unsettledResponsePorts.delete(port ?? this);

    // Send the method response (or exception) to the user thread.
    try {
      (port ?? this).postMessage(
      ...
      (port ?? this).postMessage(wrapMessage('error', exception));
    }
...
module.exports = function setupModuleWorker(workerData, syncCommPort) {
  ...
  /**
   * @type {MessagePort} This is a message port to the real main thread; it purposefully does not use the special worker
   * `parentPort` for reusability reasons around having the same code for worker_thread and main thread communication.
   */
  doListenTo(workerData.listenTo)
  // prevent parent port to main thread from keeping hooks thread alive
  syncCommPort.unref()
  ...
  return PromisePrototypeThen(
    customizedModuleWorker(lock, listenTo, errorHandler),
    undefined,
    errorHandler,
  );
};
