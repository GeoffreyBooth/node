...
class Worker extends EventEmitter {
  constructor(filename, options = kEmptyObject) {
    ...
    const { port1: toWorkerThread, port2: toHooksThread } = commsChannel;
    transferList.push(toWorkerThread);
    ...
    if (!isInternal) {
      // This is intentionally ignoring the return value
      // require('internal/modules/esm/loader').getHooksProxy().makeAsyncRequest(
      //   '#listenTo', [toHooksThread], toHooksThread
      // );
    }
...
module.exports = {
  ...
  SHARE_ENV,
  /**
   * Populated by worker_thread main to provide a way to reference a shared single
   * hooks thread. This MessagePort will send messages into the hooks worker.
