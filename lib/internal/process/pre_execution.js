 ...
 * Replace `process.argv[1]` with the resolved absolute URL or file path of the entry point, if requested and a
 * replacement can be found; preserving the original value as `process.argv1`.
 * @param {boolean} expandArgv1 - Whether to replace `process.argv[1]` with the resolved absolute URL or file path of
...
function patchProcessObject(expandArgv1) {
  ...
  if (expandArgv1) {
    ...
    if (getOptionValue('--experimental-default-type') === 'module') {
      try {
        // Per determineMainEntry, in ESM mode mainEntry is a URL instance.
        if (mainEntry.protocol === 'file:') {
          const { fileURLToPath } = require('internal/url');
          process.argv[1] = fileURLToPath(mainEntry);
        } else {
          // If argv[1] is not a file URL, remove it from argv.
          // This matches the behavior of other non-file inputs such as --eval.
          ArrayPrototypeSplice(process.argv, 1, 1);
        }
      } catch {
        // Continue regardless of error.
      }
    } else { // Legacy mode, --experimental-default-type=commonjs
      try {
        // Per determineMainEntry, in legacy mode mainEntry is a path string.
...
/**
 * For non-STDIN input, we need to resolve what was specified on the command line into an absolute URL or path.
 * In ESM mode, we parse the main entry point as a URL. Take the input and generate an absolute URL from it.
 * In legacy mode, we parse the main entry point as a path. Take the input and generate an absolute path from it.
...
function determineMainEntry(input = process.argv[1]) {
  ...
  if (getOptionValue('--experimental-default-type') === 'module') {
    // In ESM mode, we parse the main entry point as a URL.
    const { URL } = require('internal/url');
    const { getCWDURL } = require('internal/util');
    return new URL(input, getCWDURL());
  }
  // Legacy mode, --experimental-default-type=commonjs
